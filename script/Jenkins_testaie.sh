...
if [ "$IS_PR_BUILD" = true ]; then
  ...
  # Clean up any previous aie-baremetal_taie directory
  if [ -d "aie-baremetal_taie" ]; then
    echo "Removing existing aie-baremetal_taie directory..."
    rm -rf aie-baremetal_taie
  fi

  # Clone the pull request's source repo
  git clone $GIT_URL aie-baremetal_taie &&
  cd aie-baremetal_taie &&
...
# Set up the environment and build the whole project
...
make clean
# make AIE_KERNEL_LOC=./testaie/kernel AIE_GEN=2 -j32
