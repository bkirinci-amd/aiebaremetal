...
# QEMU Automation Test for testkernel_rtp_sync example
...
TEST_NAME="testkernel_rtp_sync QEMU Test"
...
# Allow up to 10 min for QEMU run on CI (slow nodes). Override with QEMU_TIMEOUT env if needed.
QEMU_TIMEOUT="${QEMU_TIMEOUT:-600}"
# ELF name depends on platform (vck190 or vek280); detect after build
ELF_NAME=""
...
# Verbose logs by default. Set JENKINS_QEMU_QUIET=1 for quiet CI mode (FileLogStorage EOF / size limits).
BUILD_QUIET=false
QEMU_QUIET=false
...
if [ "$SUCCESS" != true ]; then
    qemu_log_hint "Inspect example/testkernel_rtp_sync/qemu_test_tail.txt and Jenkins console above [STAGE 4]."
fi
