...
void OneInput(input_window<int32> *__restrict dataIn, output_window<int32> *__restrict aOut,
    ...
    printf("OneInput currentRow %d currentCol %d\n", currentRow, currentCol);
    wlog("oneinput 1 ", 1);
    ...
    for (unsigned i = 0; i < NUM_A_ELMNTS_PER_TILE * itr / WIN_SIZE; i++) {
        ...
        window_acquire(dataIn);
        for (unsigned w = 0; w < WIN_SIZE / VECTOR_LENGTH; w++)
            chess_prepare_for_pipelining { window_writeincr(aOut, window_readincr_v<VECTOR_LENGTH>(dataIn)); }
        window_release(dataIn);
        window_release(aOut);
    }
    ...
    for (unsigned i = 0; i < NUM_COLS; i++) {
        ...
        for (unsigned k = 0; k < NUM_ROWS_PER_TILE; k++) {
            ...
            for (unsigned l = 0; l < NUM_COLS / VECTOR_LENGTH; l++) {
                aie::vector<int32, VECTOR_LENGTH> va =
