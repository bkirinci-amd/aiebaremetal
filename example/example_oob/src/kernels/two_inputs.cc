...
void TwoInputs(input_window_uint32 *dataIn, input_window_uint32 *bypassResult, output_window_uint32 *dataOut,
    ...
    printf("two input currentCol = %d start\n", currentCol);
    for (unsigned i = 0; i < NUM_A_ELMNTS_PER_TILE / WIN_SIZE; i++) {
        printf("two input cur i = %d currentCol = %d\n", i, currentCol);
    ...
    for (unsigned i = 0; i < NUM_A_ELMNTS_PER_TILE * (NUM_HW_COLS - currentCol - 1) / WIN_SIZE; i++) {
        ...
        printf("two input forward i = %d currentCol = %d\n", i, currentCol);
        window_acquire(dataOut);
        window_acquire(dataIn);
        for (unsigned w = 0; w < WIN_SIZE / VECTOR_LENGTH; w++)
            chess_prepare_for_pipelining { window_writeincr(dataOut, window_readincr_v<VECTOR_LENGTH>(dataIn)); }
        window_release(dataIn);
...
/* read 1 entire column of b */
        ...
        for (unsigned w = 0; w < (NUM_COLS / WIN_SIZE); w++) {
            ...
            for (unsigned x = 0; x < (WIN_SIZE / VECTOR_LENGTH); x++)
                chess_prepare_for_pipelining {
                    aie::store_unaligned_v(b + (w * WIN_SIZE) + (x * VECTOR_LENGTH),
                                           window_readincr_v<VECTOR_LENGTH>(dataIn));
        ...
        for (unsigned k = 0; k < NUM_ROWS_PER_TILE; k++) {
            ...
            for (unsigned l = 0; l < NUM_COLS / VECTOR_LENGTH; l++)
                chess_prepare_for_pipelining {
                    aie::vector<uint32_t, VECTOR_LENGTH> va =
                        aie::load_v<VECTOR_LENGTH>(a + (k * NUM_COLS) + (l * VECTOR_LENGTH));
